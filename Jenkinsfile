...
podTemplate(
    ...
    node(label) {
        def githubRepository = "https://github.com/Starwars2020/c-api-01.git"
        def githubCredential = "github-api-token"
        def dockerRegistry = "localhost:5000"
        def dockerRegistryCredential = ""
        def dockerImageName = "localhost:5000/c-api-01-app"
        ...
        stage('Run kubectl') {
            container('kubectl') {
                //sh "kubectl apply -f c-api-01-app-deployment.yaml -n ${nameSpace}"
                //sh "kubectl apply -f c-api-01-app-service.yaml -n ${nameSpace}"
                //sh "kubectl apply -f c-api-01-app-ingress.yaml -n ${nameSpace}"
            }
        }
    }
}
